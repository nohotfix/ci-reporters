# Dogfood — Jest app using `@nohotfix/jest-reporter`

The canonical acceptance for the Jest reporter (the same bar the Playwright/Vitest/Cypress dogfoods
set): an `nhf`-tagged Jest suite that reports into a **live** NoHotfix and flips a real automated
...
pnpm --filter @nohotfix/example-jest-app dogfood
```
...
```bash
NOHOTFIX_DRY_RUN=true pnpm --filter @nohotfix/example-jest-app dogfood
...
```bash
pnpm --filter @nohotfix/example-jest-app dogfood --shard=1/3
```
