...
/**
 * Live E2E harness for the vitest-reporter.
 *
 ...
 * The reporter (wired in vitest.config.ts) then submits this test's result to the live run; the
...
test.skipIf(!ciKey)('reports a seeded automated result to a live NoHotfix run', (ctx) => {
  // Bind this test to the target run's automated member via task metadata — the whole integration.
  nhf.tag(ctx, ciKey!);
